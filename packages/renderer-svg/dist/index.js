...
function shapeEl(p, shape, fill, opacity) {
...
export function renderStaticSvg(grid, options = {}) {
    const opts = { ...DEFAULTS, ...options };
    const w = grid.width + opts.padding * 2;
    const h = grid.height + opts.padding * 2;
    const bg = opts.transparent || opts.background === "transparent"
        ? ""
 ...
 */
export function renderAnimatedSvg(grid, animation, options = {}) {
    const opts = { ...DEFAULTS, ...options };
    const w = grid.width + opts.padding * 2;
    const h = grid.height + opts.padding * 2;
    const bg = opts.transparent || opts.background === "transparent"
...
export function frameOpacities(animation, dotIndex) {
